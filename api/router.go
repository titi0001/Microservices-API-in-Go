...
func SetupAuthServer(host, serviceURL string, dbClient *sqlx.DB) *http.Server {
	...
	router.
		...
		Name("AuthLogin")
	router.
		HandleFunc("/auth/register", authHandler.Register).
		Methods(http.MethodPost).
		Name("AuthRegister")
	router.
		HandleFunc("/auth/refresh", authHandler.Refresh).
...
func setupRoutes(
	...
	publicRouter.HandleFunc("/auth/login", NewAuthHandler(authService).Login).
		Methods(http.MethodPost).
		Name("AuthLogin")
	publicRouter.HandleFunc("/auth/register", NewAuthHandler(authService).Register).
		Methods(http.MethodPost).
		Name("AuthRegister")
	publicRouter.HandleFunc("/auth/refresh", NewAuthHandler(authService).Refresh).
		Methods(http.MethodGet).
		Name("AuthRefresh")
	...
	protectedRouter.
		HandleFunc("/auth/verify", NewAuthHandler(authService).Verify).
		Methods(http.MethodGet).
		Name("AuthVerify")

	protectedRouter.
		HandleFunc("/customers", NewCustomerHandler(customerService).GetAllCustomers).
		Methods(http.MethodGet).
		Name("GetAllCustomers")

	protectedRouter.
		HandleFunc("/customers/{customer_id:[0-9]+}", NewCustomerHandler(customerService).GetCustomer).
		Methods(http.MethodGet).
		Name("GetCustomer")

	protectedRouter.
		HandleFunc("/customers/{customer_id:[0-9]+}/account", NewAccountHandler(accountService).NewAccount).
		Methods(http.MethodPost).
		Name("NewAccount")

	protectedRouter.
		HandleFunc("/customers/{customer_id:[0-9]+}/account/{account_id:[0-9]+}", NewAccountHandler(accountService).MakeTransaction).
		Methods(http.MethodPost).
		Name("NewTransaction")

	protectedRouter.
		HandleFunc("/permissions", NewPermissionsHandler(authService).GetRolePermissions).
		Methods(http.MethodGet).
